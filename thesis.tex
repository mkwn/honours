...
I tried to add value with clearer explanation and filling in skipped
steps, or by adapting (and simplifying) the proof for a special case.

\begin{note}Should I explain in more detail what I've added to specific
proofs? \end{note}
...
defintions. I've decided to describe Stein's original approach \cite{Ste86}
because, in particular, it does a better job motivating the method
of exchangeable pairs in \ref{sec:exchangeable-pairs}.
...
In practice it's a little unwieldy to show an operator is characterizing
...
\[
\E\left[\left|\X_{\N}\right|^{k}\left|\T_{\N}\fo\left(\X_{\N}\right)\right|\right]\le\E\left[\left|\X_{\N}\right|^{k}\left|\fo'\left(\X_{\N}\right)\right|\right]+\E\left[\left|\X_{\N}\right|^{k+1}\left|\fo\left(\X_{\N}\right)-\fo\left(0\right)\right|\right]+\left|\fo\left(0\right)\right|\E\left[\left|\X_{\N}\right|^{k+1}\right].
\]
...
Note that 
\begin{eqnarray*}
\int_{t}^{\infty}x^{k+1}e^{-x^{2}/2} & \le & e^{-t^{2}/2}\int_{0}^{\infty}\left(y+t\right)^{k+1}e^{-y^{2}/2}\\
 & \prec & e^{-t^{2}/2}\left(1+t+\dots t^{k+1}\right)\\
 & \prec & e^{-t^{2}/2}\left(1+t^{k+1}\right),
\end{eqnarray*}
with the substitution $y=x-t$. With Fubini's theorem (see \cite[Theorem 8.8]{Rud66}),
\begin{alignat*}{1}
\E\left[\left|\X_{\N}\right|^{k+1}\left|\fo\left(\X_{\N}\right)\right|\right] & \prec\int_{0}^{\infty}x^{k+1}\left|\int_{0}^{x}\fo'\left(t\right)\d t\right|e^{-x^{2}/2}\d x\\
 & \qquad-\int_{-\infty}^{0}x^{k+1}\left|\int_{0}^{x}\fo'\left(t\right)\d t\right|e^{-x^{2}/2}\d x\\
 & \le\int_{0}^{\infty}\int_{t}^{\infty}x^{k+1}\left|\fo'\left(t\right)\right|e^{-x^{2}/2}\d x\d t\\
 & \qquad-\int_{-\infty}^{0}\int_{-\infty}^{t}x^{k+1}\left|\fo'\left(t\right)\right|e^{-x^{2}/2}\d x\d t\\
 & \prec\int_{0}^{\infty}e^{-t^{2}/2}\left(1+t^{k+1}\right)\left|\fo'\left(t\right)\right|\d t\\
 & \qquad-\int_{-\infty}^{0}e^{-t^{2}/2}\left(1+t^{k+1}\right)\left|\fo'\left(t\right)\right|\d t.\\
 & \prec\E\left[\left|\X_{\N}\right|^{0}\left|\fo'\left(\X_{\N}\right)\right|\right]+\E\left[\left|\X_{\N}\right|^{k+1}\left|\fo'\left(\X_{\N}\right)\right|\right]\\
 & <\infty.
\end{alignat*}
...
I'm using $f\prec g$ to mean $\left|f\right|\le c\left|g\right|$
for some $c$, because I don't want to keep track of a lot of constants
when I'm only proving something is finite. Is there some existing
common notation for this?
...
\E\left[\left|\X_{\N}\right|^{k+1}\left|\U_{\N}\h\left(\X_{\N}\right)\right|\right] & \prec\int_{0}^{\infty}x^{k+1}\int_{x}^{\infty}\left|\h\left(t\right)-\E_{\N}\h\right|e^{-t^{2}/2}\d t\d x\\
 ...
 & \qquad-\int_{-\infty}^{0}x^{k+1}\int_{t}^{0}\left|\h\left(t\right)-\E_{\N}\h\right|e^{-t^{2}/2}\d x\d t\\
 & \prec\int_{-\infty}^{\infty}\left|t\right|^{k+2}\left|\h\left(t\right)-\E_{\N}\h\right|e^{-t^{2}/2}\d x\d t<\infty,
\end{alignat*}
so $\U_{\N}\h\in\cX_{\N}$. Finally, 
\[
\E\left[\left|\X_{\N}\right|^{k}\left|\left(\U_{\N}\h\right)'\left(\X_{\N}\right)\right|\right]\le\E\left[\left|\X_{\N}\right|^{k}\left|\h\left(\X_{\N}\right)\right|\right]+\E\left|\X_{\N}\right|^{k+1}\E_{\N}\h+\E\left[\left|\X_{\N}\right|^{k+1}\left|\X_{\N}\U_{\N}\h\left(\X_{\N}\right)\right|\right]
\]
since $\U_{\N}\h$ satisfies \ref{eq:diff-eqn}, so $\U_{\N}\h\in\cF_{\N}$.\end{proof}
\begin{rem}
...
\ref{thm:poisson-characterizing}, then 
\[
...
for all $x\in\RR$, and it follows that $\left|\E_{\N}\h\right|\le\E\left|\X_{\N}\right|=\frac{2}{\sqrt{2\pi}}\int_{0}^{\infty}xe^{-x^{2}/2}\d x=\sqrt{2/\pi}$.
Then, for $x\ge0$,
...
argument gives $\left|\U_{\N}\h\left(x\right)\right|\le2$ for $x\le2$,
...
\]
Set $g=\h'+\U_{\N}\h\in\cX_{\N}$ (note $\left\Vert g\right\Vert _{\infty}\le\left\Vert \h'\right\Vert _{\infty}+\left\Vert \U_{\N}\h\right\Vert _{\infty}\le3$),
so that 
\[
\E_{\N}g=\E\left[\left(\U_{\N}\h\right)''\left(\X_{\N}\right)-\X_{\N}\left(\U_{\N}\h\right)'\left(\X_{\N}\right)\right]=0
\]
as in \ref{eq:normal-stein-identity}. So,
\[
...
\]
for some $c$, by the general form \ref{eq:diff-eqn-soln} of the
solution to \ref{eq:diff-eqn}. Since $\left(\U_{\N}\h\right)'\in\cX_{\N}$,
we must have $e^{-x^{2}/2}\left(\U_{\N}\h\right)'\left(x\right)\to0$.
That is, $c=0$. For $x\ge0$, we have
...
\left|\left(\U_{\N}g\right)'\left(x\right)\right| & =\left|g\left(x\right)+x\U_{\N}g\left(x\right)\right|\\
 & \le3+3xe^{x^{2}/2}\int_{x}^{\infty}e^{-t^{2}/2}\\
 & \le3+3xe^{x^{2}/2}\int_{x}^{\infty}\frac{t}{x}e^{-t^{2}/2}\\
 & =6.
\end{alignat*}
...
The Berry-Esseen theorem is a quantitative version of the central
limit theorem. The original version was independently proved in \cite{Ber41}
and \cite{Ess42} with some relatively involved Fourier analysis.
However, the machinery of Stein's method allows for a very simple
proof.

The purpose of this section is to make the theory a bit more concrete
by giving our first example of Stein's method in action. Of course,
the result is also of practical interest in statistics.
\begin{thm}
...
\begin{rem}
In the following proof, we will prove the case $C=9$. The sharper
bounds in \ref{rem:better-wasserstein-bounds} yield $C=3$ with the
same argument.
\end{rem}
...
theorem is usually stated for the Kolmogorov metric, because it has
a direct interpretation as the supremum difference between probabilities.
However in this case the transfer makes the bound much worse. It is
possible to use Stein's method directly in the Kolmogorov metric (see
\cite{Ste12}), but this is a little more involved.\end{rem}
\begin{proof}
...
of $\cY$, we have 
\begin{alignat*}{1}
\E\left|Q_{\i}^{3}\frac{f''\left(E\right)}{2}\right| & \le3\E\left|Q^{3}\right|,\\
\E\left|Q_{\i}f''\left(E'\right)\right| & \le6\E\left|Q\right|
\end{alignat*}
...
metric. We can bound the functions in $\U_{\Po\left(\lambda\right)}\cH_{\Po\left(\lambda\right)}$
...
\left|\U_{\Po\left(\lambda\right)}\one_{\A}\left(\i\right)\right| & \le\frac{e^{\lambda}\,\left(\i-1\right)!}{\lambda^{\i}}\Po_{\lambda}\left(\left[\i,\infty\right]\right)\\
 ...
 & \le2.
\end{alignat*}
This also shows $\left|\U_{\Po\left(\lambda\right)}\one_{\A}\left(\i\right)\right|\le2\lambda^{-1/2}$
...
\left|\U_{\Po\left(\lambda\right)}\one_{\A}\left(\i\right)\right| & =\i^{-1}\sum_{\iii=0}^{\floor{\lambda^{1/2}}}b_{\iii}+\i^{-1}\sum_{\iii=\ceil{\lambda^{1/2}}}^{\infty}b_{\iii}\nonumber \\
 ...
 & \le3\lambda^{-1/2}.\nonumber 
\end{alignat}
We have proved $\left\Vert \U_{\Po\left(\lambda\right)}\h\right\Vert _{\infty}\le\min\left\{ 2,3\lambda^{-1/2}\right\} $
for $\h\in\cH$.
\end{proof}
\begin{note}
...
\]
for all $k$. Finally, note $\U_{\Po\left(\lambda\right)}\one_{\NN\backslash\A}+\U_{\Po\left(\lambda\right)}\one_{\A}=\U_{\Po\left(\lambda\right)}\one_{\NN}=0$,
so 
\[
...
\]
We conclude $\left\Vert \U_{\Po\left(\lambda\right)}\h\right\Vert _{\infty}\le\lambda^{-1}\left(1-e^{-\lambda}\right)$
for $\h\in\cH$.\end{proof}
\begin{rem}
A similarly elementary but more fiddly argument can be used to show
that the bound $\left\Vert \fo\right\Vert _{\infty}\le\min\left\{ 1,1.4\lambda^{-1/2}\right\} $
is also valid (see \cite[Lemma 4]{Bar83}). A significantly more sophisticated
...
method, that is effective in wide generality for discrete random variables.
It is possible to adapt the method to non-discrete random variables,
but in what follows, we assume $\X$ is discrete. Let $\OmX$ be the
support of $\X$.
\begin{example}
...
for exchangeable pairs is to piggyback onto results proved using combinatorial\emph{
switchings.} In this subsection, I'll describe how a certain Poisson
limit theorem concerning short cycles in random regular graphs was
improved by applying Stein's method in a quite natural, and generally
applicable, way. First, we need some background on short cycles in
random regular graphs, and switchings.
...
is odd.\end{thm}
\begin{rem}
The full theorem is that for any $r$, the vector of short cycle counts
$\left(\X_{1,n}^{\left(d\right)},\dots\X_{r,n}^{\left(d\right)}\right)$
...
\begin{defn}
\label{def:cycle-switching}Let $C=v_{1}\dots v_{\ell}$ be a short
cycle in a $d$-regular graph $G$. For each $\i\in\ZZ/\ell\ZZ$,
...
this operation does not create or delete any short cycle except $C$.
Then, we say $G\rightsquigarrow G'$.
\end{defn}
\begin{todo}
...
In \cite[Section 3]{MWW04}, bounds were obtained for the ``average
number of ways to switch out'' of each graph $G$ (that is, the number
of graphs $G'$ satisfying $G\rightsquigarrow G'$), and similarly
the ``number of ways to switch in'' to each graph. This gives an
estimate on the relative sizes of $S\left(k\right)$ and $S\left(k-1\right)$
for each $k$, which is then used to estimate the distribution of
$\X_{\ell,n}^{\left(d\right)}$. The relevant estimates work in the
regime $\left(d-1\right)^{2\ell-1}=o\left(n\right)$.
...
We define a weighted multigraph $\fG$ with vertex set $\Om$. Make
an edge in $\fG$ for every pair $\left(G,G'\right)$ with $G\rightsquigarrow G'$.
