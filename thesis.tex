...
The University of New South Wales\date{June 5, 2014}}
...
(Updated with corrections on \today)
\par\end{center}
...
\textbf{l}imit \textbf{t}heorem} says that the number of occurrences
of independent ``rare'' events is ``approximately Poisson''. These
...
\addcontentsline{toc}{subsection}{Notation and Assumed Knowledge}
...
\protect\caption{\label{fig:graph-conventions}From left to right: a multigraph that
...
will be useful. This chapter will assume knowledge of basic measure
theory; see, for example, \cite{Rud66}. Where possible, we will note
any simplifications that arise from assuming discreteness.
...
$\L{\X}\in\P\left(\Om\right)$ of $\X$ with respect to an underlying
probability $\Pr$ is the pushforward measure with respect to $\X$.
That is, it is a probability measure defined by $\L{\X}\left(\A\right)=\Pr\left(\X^{-1}\left(\A\right)\right)=\Pr\left(\X\in\A\right)$
for $\A\subseteq\cA$. For ease of reading we occasionally write $\Lb{\X}$
instead of $\L{\X}$.
\end{defn}
...
If $\X\in\N\left(\mu,\sigma\right)$ then it is a simple exercise
to prove that $\frac{\X-\mu}{\sigma}\in\N\left(0,1\right)$. So, we
will sometimes only consider the \emph{standard normal} distribution
...
The \emph{variance} of $\X$ is a measure of how much variation there
is from the expected value; it is defined by $\Var\X=\E\left(\X-\E\X\right)^{2}=\E\X^{2}-\left(\E\X\right)^{2}$.\end{defn}
\begin{rem}
\label{rem:indicator-expectation}The expected value of an indicator
random variable $\one_{\A}$ is just $\Pr\left(\A\right)$. If $\X$
is a random variable that can be interpreted as counting the number
of objects that satisfy some property, then we can express $\X$ as
a sum of indicator variables $\sum_{\i}\one_{\A_{\i}}$, where $\A_{\i}$
is the event that the $\i$th object satisfies our property. Noting
that $\E$ is linear, we have $\E\X=\sum_{\i}\E\one_{\A_{\i}}=\sum_{\i}\Pr\A_{\i}$.
\end{rem}
If a random variable $\X$ has underlying probability space $\left(\Om_{1},\cA_{1},\Pr\right)$,
then by immediate properties of the pushforward measure, we have $\E\X=\int\X\left(\om\right)\d\Pr\left(\om\right)$.
It follows that if $\X:\left(\Om_{1},\cA_{1},\Pr\right)\to\left(\Om_{2},\cA_{2}\right)$
is an arbitrary random element, and $h\in L^{1}\left(\Om_{2},\cA_{2}\right)$,
then $\E\left[h\left(\X\right)\right]=\int h\left(x\right)\L{\X}\left(x\right)$.

If $\X$ is discrete, we obtain the well-known formula 
\begin{equation}
\E\left[h\left(\X\right)\right]=\sum_{x\in\supp\X}h\left(x\right)\,\Pr\left(\X=x\right).\label{eq:discrete-expectation}
\end{equation}

\begin{example}
If $\X\in\Po\left(\lambda\right)$, then \ref{eq:discrete-expectation}
and the Taylor expansion of the exponential can be used to show that
$\E\X=\lambda$ and $\E\left[\X\left(\X-1\right)\right]=\lambda^{2}$,
so $\Var\X=\lambda$.
\end{example}
There is a ``continuous analogue'' of \ref{eq:discrete-expectation}
for certain random variables.
\begin{defn}
The \emph{distribution function} $\F_{\X}$ of a random variable $\X$
is defined by\\
$\F_{\X}\left(x\right)=\Pr\left(\X\le x\right)$ for all $x\in\RR$.
If (for all $x\in\RR$), $\F_{\X}\left(x\right)=\int_{-\infty}^{x}f_{\X}\left(x\right)\d x$
for some $f_{\X}$, then $f_{\X}$ is called a \emph{Lebesgue density}
of $\X$, and $\X$ is called a \emph{continuous} random variable.
\end{defn}
If $\X$ is a continuous random variable, then it follows from the
measure-theoretic definition of the integral that 
\begin{equation}
\E\left[h\left(\X\right)\right]=\int_{\RR}h\left(x\right)f_{\X}\left(x\right)\d x.\label{eq:continuous-expectation}
\end{equation}

\begin{example}
If $\X\in\N\left(\mu,\sigma\right)$, then $\left(2\pi\right)^{-1/2}e^{-x^{2}/2}$
is a Lebesgue density of $\X$. We can use \ref{eq:continuous-expectation}
together with integration by parts and the Gaussian integral to establish
$\E\X=\mu$ and $\Var\X=\sigma^{2}$.
\end{example}
If we fix a particular underlying probability space $\left(\Om,\cA,\Pr\right)$,
we can view expectation as a linear functional $\E_{\Pr}:L^{1}\left(\Om,\Pr\right)\to\RR$,
satisfying $\E_{\Pr}h=\int h\left(\om\right)\d\Pr$. For a random
variable $\X$, define $\E_{\X}=\E_{\Lb{\X}}$, so that $\E_{\X}h=\E h\left(\X\right)$
for all $h\in L^{1}\left(\RR,\Lb{\X}\right)$.
...
probability. The expected value of the distribution $\L{\X|\A}$ is
called the \emph{conditional expected value of $\X$ given $\A$}
and is denoted $\E\left[\X|\A\right]$.
\end{defn}
...
Then, $\m:\A\mapsto\E\left[\X_{1}\one_{\A}\right]$ is a signed measure
on $\left(\Om,\sigma\left(\X_{2}\right)\right)$ that is absolutely
continuous with respect to the restriction of $\Pr$ to $\sigma\left(\X_{2}\right)$.
By the Radon-Nikodym \textbf{t}heorem (see \cite[Theorem~6.10]{Rud66})
there is an $\sigma\left(\X_{2}\right)$-measurable random variable
$\E\left[\X_{1}|\X_{2}\right]$ that satisfies $\E\left[\X_{1}\one_{\A}\right]=\E\left[\E\left[\X_{1}|\X_{2}\right]\one_{\A}\right]$
for all $\A$ in $\sigma\left(\X_{2}\right)$. This random variable
is almost uniquely defined: for any two choices of $\E\left[\X_{1}|\X_{2}\right]$,
the probability that they differ is zero.
\begin{defn}
...
\[
\E^{\X_{2}}\,:\, L^{1}\left(\Om,\cA,\Pr\right)\to L^{1}\left(\Om,\sigma\left(\X_{2}\right),\Pr\right)\,:\,\X_{1}\mapsto\E\left[\X_{1}|\X_{2}\right].
\]
\end{defn}
\begin{rem}
...
\begin{rem}
Since $\one_{\A_{1}}\one_{\A_{2}}=\one_{\A_{1}\cap\A_{2}}$, note
that $\E\left[\left(\E^{\X_{2}}\X_{1}\right)\one_{\A_{1}}\one_{\A_{2}}\right]=\E\left[\X_{1}\one_{\A_{1}}\one_{\A_{2}}\right]$
for all $\A_{1},\A_{2}\in\sigma\left(\X_{2}\right)$. That is, $\left(\E^{\X_{2}}\X_{1}\right)\one_{\A_{1}}=\E^{\X_{2}}\left[\X_{1}\one_{\A_{1}}\right]$.
By linearity and an approximation argument, it follows that $\left(\E^{\X_{2}}\X_{1}\right)\X_{3}=\E^{\X_{2}}\left[\X_{1}\X_{3}\one_{\A_{1}}\right]$
for all $\sigma\left(\X_{2}\right)$-measurable integrable random
variables $\X_{3}$. This is ``linearity of expectation'' for conditional
expectation.
\end{rem}
...
expectation.
\begin{prop}
\label{prop:tower-contractivity}Suppose $\X_{1}$, $\X_{2}$ and
$\X_{3}$ are random variables defined on the same underlying probability
space $\left(\Om,\cA\right)$. Further, suppose $\X_{3}$ is $\sigma\left(\X_{2}\right)$-measurable
(intuitively, this means $\X_{3}$ is dependent only on the information
in $\X_{2}$). Then\begin{enumerate}

\item (Tower Law) $\E^{\X_{3}}\left[\E^{\X_{2}}\X_{1}\right]=\E^{\X_{3}}\X_{1}$.
In particular, if we take $\X_{3}=\one_{\Om}$ then it follows that
$\E\left[\E^{\X_{2}}\X_{1}\right]=\E\left[\X_{1}\right]$.

\item (Contractivity, $L^{1}$ version) $\E\left|\E^{\X_{2}}\X_{1}\right|\le\E\left|\X_{1}\right|$.

\item (Contractivity, $L^{2}$ version) $\E\left|\E^{\X_{2}}\X_{1}\right|^{2}\le\E\left|\X_{1}\right|^{2}$.
...
we have
\[
\E\left[\left(\E^{\X_{3}}\X_{1}\right)\one_{\A}\right]=\E\left[\X_{1}\one_{\A}\right]=\E\left[\left(\E^{\X_{2}}\X_{1}\right)\one_{\A}\right].
\]
Since $\E^{\X_{3}}\X_{1}$ is $\sigma\left(\X_{3}\right)$-measurable,
...
\]
proving the $L^{1}$ version of contractivity. Finally, note $\E^{\X_{2}}\left[\X_{1}\E^{\X_{2}}\X_{1}\right]=\left(\E^{\X_{2}}\X_{1}\right)^{2}$
by linearity of expectation, so $\E^{\X_{2}}\left(\X_{1}-\E^{\X_{2}}\X_{1}\right)^{2}=\E^{\X_{2}}\X_{1}^{2}-\left(\E^{\X_{2}}\X_{1}\right)^{2}>0$
and by the tower law,
\[
\E\left|\E^{\X_{2}}\X_{1}\right|^{2}\le\E\left[\E^{\X_{2}}\X_{1}^{2}\right]=\E\left|\X_{1}\right|^{2}.
\]
We have proved the $L^{2}$ version of contractivity.
\end{proof}

\begin{rem}
Suppose $\X_{1}$ and $\X_{2}$ are discrete. Combining \ref{eq:discrete-expectation}
...
that a $d$-regular graph is a graph where each vertex has degree
$d$, where the degree of a vertex is the number of incident edges).
...
We discuss Markov chains only superficially, to the extent that is
required for \ref{sec:exchangeable-pairs}. The theory of Markov chains
(and Markov processes in general) is a very rich area of probability
theory, see for example \cite[Chapter~7]{Kal02}.

For our purposes, a (two-sided) \emph{Markov chain} is a sequence
of discrete random elements $\left(X_{\i}\right)_{\i\in\ZZ}$ taking
values on a common set $\Om$ (the \emph{state space}), such that
\[
...
\begin{defn}
A Markov chain $\left(X_{\i}\right)_{\i\in\ZZ}$ is \emph{time-homogeneous}
if
\begin{equation}
...
Markov chain does not depend on the current point in time, given the
current state. The probability in \ref{eq:time-homogeneous-defn}
is called the \emph{transition probability} from $x$ to $y$, and
is denoted $p_{xy}$ or $p\left(x,y\right)$.
\begin{defn}
A time-homogeneous Markov chain $\left(X_{\i}\right)_{\i\in\ZZ}$
...
\begin{equation}
\pi_{y}=\Pr\left(X_{\i}=y\right)=\sum_{x\in\Om}\Pr\left(X_{\i}=y\mbox{ and }X_{\i-1}=x\right)=\sum_{x\in\Om}\pi_{x}p_{xy}\label{eq:stationary-defn}
\end{equation}
for all $y\in\Om$. In fact, the distribution $\Lb{X_{\i}}_{\i\in\ZZ}$
of a stationary Markov chain is uniquely defined by a stationary distribution
$\L X$ and a set of transition probabilities $\left\{ p_{xy}\right\} _{x,y\in\Om}$
which satisfy \ref{eq:stationary-defn} for all $y\in\Om$ and $\sum_{y\in\Om}p_{xy}=1$
for all $x\in\Om$.
\begin{defn}
A stationary Markov chain $\left(X_{\i}\right)_{\i\in\ZZ}$ is \emph{reversible}
if the distribution of the sequence $\left(X_{\i}\right)_{\i\in\ZZ}$
is the same as the distribution of the sequence $\left(X_{-\i}\right)_{\i\in\ZZ}$ 
\end{defn}
Reversibility means that a Markov\textbf{ }chain can be equally well
...
for all $x,y\in\Om$.
\begin{defn}
We say a time-homogeneous Markov chain $\left(X_{\i}\right)_{\i\in\ZZ}$
...
Markov chain on its vertex set $V$, which is stationary with respect
to the uniform distribution on $V$.\end{prop}
\begin{proof}
...
$\left\{ p_{xy}\right\} _{x,y\in V}$ define a reversible Markov chain
stationary with respect to $\L X$. \end{proof}
\begin{example}
Let $G$ be a cycle of length $n$, with a weight of 1 on each edge.
The corresponding Markov chain is a random walk: at each time-step,
there is equal probability of moving one step clockwise or counterclockwise
around the cycle.
\end{example}
...
we say $\Lb{\X_{n}}$ converges \emph{weakly} to $\Lb{\X}$, and write
$\Lb{\X_{n}}\to\Lb{\X}$.
\end{defn}
...
\begin{thm}
[Poisson \textbf limit \textbf theorem]Let $\L n$ be the distribution
of a zero-one random variable with $n\L n\left(1\right)\to\lambda$.
For each $n\in\NN$, let $Q_{1}^{\left(n\right)},Q_{2}^{\left(n\right)},\dots\in\L Q$
be independent random variables and let 
\[
\X_{n}=\sum_{\i=1}^{n}Q_{\i}^{\left(n\right)}.
\]
...
in the weak-star topology. The subspace topology corresponding to
the set of real probability distributions $\P\left(\RR\right)$ is
confusingly called the \emph{weak topology} on probability distributions.

Although $C_{b}\left(\RR\right)^{*}$ is not metrizable, the unit
ball (in dual space norm) of $C_{b}\left(\RR\right)^{*}$ is in fact
...
\begin{defn}
\label{def:general-metrics}Let $\cH$ be a determining class (\ref{def:determining-class})
of real measurable ``test'' functions that are uniformly bounded.
...
$x\in\RR$), and have Lipschitz modulus at most 1 (that is, $\left|\h\left(x_{1}\right)-\h\left(x_{2}\right)\right|\le\left|x_{1}-x_{2}\right|$
...
\item If $\cH_{\W}$ is the set of functions with Lipschitz modulus
...
the weak topology.
\end{thm}
We will need a small lemma to prove \ref{thm:BL-metrizes-weak}. This
is a special case of Prokhorov's \textbf{t}heorem (\cite[Theorem~14.3]{Kal02}).
\begin{lem}
...
\item \label{prop:transfer-K/W}If $\X_{2}$ is continuous and has
density $f_{\X_{2}}$ satisfying $\left\Vert f_{\X_{2}}\right\Vert _{\infty}\le C$
for some $C>0$, then $d_{\K}\left(\X_{1},\X_{2}\right)\le\sqrt{2Cd_{\BL}\left(\X_{1},\X_{2}\right)}$.
...
\begin{example}
If $\X_{2}\in\N\left(0,1\right)$ then $\X_{2}$ has density given
by $f_{\X_{2}}\left(x\right)=\left(2\pi\right)^{-1/2}e^{-x^{2}/2}$.
We can take $C=\left(2\pi\right)^{-1/2}$ to obtain $d_{\K}\left(\X_{1},\X_{2}\right)\le\left(2/\pi\right)^{1/4}\sqrt{d_{\BL}\left(\X_{1},\X_{2}\right)}$.
\end{example}
...
In the next two chapters, we will present characterizing operators
...
\end{alignat*}
\begingroup\allowdisplaybreaks with the substitution $y=x-t$. With
Fubini's theorem (see \cite[Theorem~8.8]{Rud66}),
\begin{alignat*}{1}
...
and $\Lo=\N\left(0,1\right)$. Let $\U_{\N}$ be the Stein transform
defined in \ref{thm:normal-characterizing}. Then, for all $\fo\in\U_{\N}\cH$,
\[
\left\Vert \fo\right\Vert _{\infty}\le2,\quad\left\Vert \fo'\right\Vert _{\infty}\le3\sqrt{\pi/2},\quad\left\Vert \fo''\right\Vert _{\infty}\le6.
\]
\end{thm}
\begin{proof}
...
\[
\left|\E_{\N}\h\right|\le\E\left|\X_{\N}\right|=\frac{2}{\sqrt{2\pi}}\int_{0}^{\infty}xe^{-x^{2}/2}\d x=\sqrt{\frac{2}{\pi}}.
\]
Then, for $x\ge0$,
\begin{alignat*}{1}
...
so $g\in\cX_{\N}$. Now, by the definition of $\T_{\N}$ and \ref{eq:U_0-characterizing},
note that
\[
\h'\left(x\right)=\left(\T_{\N}\U_{\N}\h\right)'\left(x\right)=\left(\U_{\N}\h\right)''\left(x\right)-x\left(\U_{\N}\h\right)'\left(x\right)-\U_{\N}\h\left(x\right),
\]
so $g=\T_{\N}\left(\left(\U_{\N}\h\right)'\right)$. As in \ref{eq:normal-stein-identity},
we have 
\[
...
\]
That is, $\left(\U_{\N}\h\right)'$ satisfies \ref{eq:diff-eqn} (with
...
\]
(As before, $s=t-x$). A similar calculation shows that $\left|\left(\U_{\N}\h\right)'\left(x\right)\right|\le3\sqrt{\pi/2}$
for $x\le0$, so 
\[
...
crude. The stronger bounds $\left\Vert \fo'\right\Vert _{\infty}\le\sqrt{\pi/2}$
...
with $\E Q=0$ and $\E Q^{2}=1$ and $\E\left|Q\right|^{3}<\infty$.
Fix $n$ and define 
\[
...
\begin{rem}
In the following proof, we will prove the theorem with $C=9$. The
...
\end{equation}
with $\E f'\left(\X\right)$. For each $\i$ define $W_{\i}=\X-\frac{1}{\sqrt{n}}Q_{\i}$,
so $W_{\i}$ and $Q_{\i}$ are independent. Now, fix $\i$. Note that
$\E Q_{\i}f\left(W_{\i}\right)=\E Q_{\i}\E f\left(W_{\i}\right)=0$,
so 
\begin{equation}
...
\end{equation}
We now use a Taylor expansion with the Lagrange form of the remainder.
Since $\X-W_{\i}=\frac{1}{\sqrt{n}}Q_{\i}$, we have
\[
...
\end{alignat*}
With \ref{eq:stein-distance-transform}, this completes the proof.
\end{proof}
...
as well as their first differences (define the difference operator
$\Delta$ by $\Delta\fo\left(\i\right)=\fo\left(\i+1\right)-\fo\left(\i\right)$).
\begin{thm}
\label{thm:poisson-bound}Using the Stein transform in \ref{thm:poisson-characterizing},
for all $\fo\in\U_{\Po\left(\lambda\right)}\cH_{\TV}$, 
\[
\left\Vert \fo\right\Vert _{\infty}\le\min\left\{ 2,3\lambda^{-1/2}\right\} ,\quad\left\Vert \Delta\fo\right\Vert _{\infty}\le\lambda^{-1}\left(1-e^{-\lambda}\right).
\]
\end{thm}
\begin{proof}
...
\]
If $1\le\i\le\lambda$, then\vspace{-0.3cm}
\begin{alignat*}{1}
...
\]
for $\iii>\lambda^{1/2}$. So,\vspace{-0.4cm}
\begin{alignat*}{1}
\left|\U_{\Po\left(\lambda\right)}\one_{\A}\left(\i\right)\right| & \le\lambda^{-1}\sum_{\iii=0}^{\floor{\lambda^{1/2}}}a_{\iii}+\lambda^{-1}\sum_{\iii=\ceil{\lambda^{1/2}}}^{\i-1}a_{\iii}\\
 & \le\lambda^{-1}\left(1+\lambda^{1/2}\right)+\lambda^{-1}\sum_{\iiii=1}^{\infty}\left(1-\lambda^{-1/2}\right)^{\iiii}=2\lambda^{-1/2}
\end{alignat*}
\begingroup\allowdisplaybreaks (note $\lambda\ge\i\ge1$, so $0\le1-\lambda^{-1/2}<1$).
Now, suppose that $\i>\lambda$. Then
\begin{alignat*}{1}
...
\left|\U_{\Po\left(\lambda\right)}\one_{\A}\left(\i\right)\right| & \le\i^{-1}\sum_{\iii=0}^{\floor{\lambda^{1/2}}}b_{\iii}+\i^{-1}\sum_{\iii=\ceil{\lambda^{1/2}}}^{\infty}b_{\iii}\\
 ...
 & \le3\lambda^{-1/2}.
\end{alignat*}
We have proved that $\left\Vert \U_{\Po\left(\lambda\right)}\one_{\A}\right\Vert _{\infty}\le\min\left\{ 2,3\lambda^{-1/2}\right\} $
for $\one_{\A}\in\cH$.
...
Now, we prove the bound on $\left\Vert \Delta\fo\right\Vert _{\infty}$.
...
\]
We conclude that $\left\Vert \Delta\U_{\Po\left(\lambda\right)}\one_{\A}\right\Vert _{\infty}\le\lambda^{-1}\left(1-e^{-\lambda}\right)$
for $\one_{\A}\in\cH_{\TV}$.\end{proof}
\begin{rem}
...
argument (see \cite[Remark~10.2.4]{BHJ92}) can be used to give the
sharper bound $\left\Vert \fo\right\Vert _{\infty}\le\min\left\{ 1,\sqrt{2/\left(e\lambda\right)}\right\} $.
\end{rem}
...
by Stein's method.
\begin{thm}
\label{thm:quantitative-poisson-limit}Fix $n$ and let $Q_{1},\dots,Q_{n}\in\L Q$
...
For each $\i$ define $W_{\i}=X-Q_{\i}$ , so $W_{\i}$ and $Q_{\i}$
...
\end{alignat}
Now, fix $\i$. Since $X-W_{\i}=Q_{\i}$, we have 
\begin{equation}
...
There are a number of different techniques that can be used in different
cases. In the case of nonnegative random variables such as those found
in combinatorial applications, one particularly useful technique is
\emph{size-bias coupling}, which we will discuss in this chapter.
\begin{defn}
...
In this chapter, $\s{\X}$ will always denote a random variable with
...
by that value. To be precise, in the discrete setting the definition
...
speaking, if $\L{\X}$ is the distribution of the number of balls
in a random bin, then $\Ls{\X}$ is the distribution of the number
of balls in the bin of a random ball.
\end{rem}
...
that $\left(\X^{\left(\i\right)}\right)_{\i=1}^{n}$, $\left(Q_{\i}\right)_{\i=1}^{n}$
...
will hold, so
\[
\sum_{\i=1}^{n}\Pr\left(\X=k\mbox{ and }Q_{\i}=1\right)=k\L{\X}\left(k\right).
\]
...
vertices in the graph obtained by choosing a vertex in $G$ at random
and deleting all its adjacent edges. \ref{fig:isolated-vertices}
...
\def \iN {5}
\node[vertex] (0) at (0,0){};
\node at ({90+360/\iN * (-1/2)}:0.35cm){0};
{
 \foreach \s in {1,...,\iN}
 {
  \node[vertex] (\s) at ({90+360/\iN * (\s - 1)}:1.1cm){};
  \node at ({90+360/\iN * (\s - 1)}:1.45cm){\s};
 }
}
...
(1) edge (3)
(2) edge (3);
\node[font=\Huge] at (3cm,0) {$\rightarrow$};
}
{
\path
(1) edge (2)
...
\protect\caption{\label{fig:isolated-vertices}Suppose the $G$ is the graph on the
left. The only isolated vertex is 5, so $\protect\X=1$. Now, suppose
$I=0$, so all the edges adjacent to 0 are deleted to obtain the graph
on the right with $\protect\s{\protect\X}=3$ isolated vertices. The
new isolated vertex 0 arose because $I$ was not isolated in $G$,
and the new isolated vertex 4 arose because 4 was a degree-1 neighbour
of 0 in $G$.}
\end{figure}
...
because the $n-1$ potential edges incident to that vertex must each
independently not be present. So if $p$ is not vanishingly small
compared to $n$ then isolated points are a ``rare event'' and we
might expect $\X$ to have an approximate Poisson distribution. If
$p$ is chosen to be a function of $n$ which vanishes at just the
right rate, then we might expect the central limit theorem to approximately
hold, so that $\X$ has an approximate normal distribution. We will
therefore return to this example throughout this chapter to illustrate
the application of size-bias coupling for both normal and Poisson
approximation.

In both cases, we will need to compute $\E\X$ and $\Var\X$, so we
do this now. For all expectation calculations in this chapter, we
...
\begin{alignat*}{1}
\E{\X \choose 2} & =\E\X^{2}/2-\E\X/2={n \choose 2}\left(1-p\right)^{2n-3}
\end{alignat*}
and
\begin{alignat}{1}
\Var\X=\E\X^{2}-\left(\E\X\right)^{2} & =n\left(n-1\right)\left(1-p\right)^{2n-3}+n\left(1-p\right)^{n-1}-n^{2}\left(1-p\right)^{2n-2}\nonumber \\
...
objective is to compare $\E\left[Wf\left(W\right)\right]$ to $\E\left[f'\left(W\right)\right]$,
...
we have $\E\left[\X f\left(\left(\X-\mu\right)/\sigma\right)\right]=\mu\E\left[f\left(\left(\s{\X}-\mu\right)/\sigma\right)\right]$
and
\[
...
\]
so by the tower law, $\E\left[\E^{\X}\left[\s{\X}-\X\right]\right]=\E\left[\s{\X}-\X\right]=\sigma^{2}/\mu$.
...
perturbation of $\X$. We show how $d_{\W}\left(\X,\N\left(0,1\right)\right)$
...
Let $\s{\X}$ be the number of isolated vertices with the edges incident
to a random vertex deleted. For each vertex $\i$ in $G$, let $D_{\i}$
be the number of vertices adjacent to $\i$ which have degree $1$,
and let $R_{\i}=1-Q_{\i}$ be the indicator random variable for the
event that $\i$ is \emph{not} isolated. Let $I$ be a (uniformly)
random vertex in $\range n$, so that 
\[
\s{\X}-\X=D_{I}+R_{I}.
\]
Let $\L D$ be the common distribution of each $D_{\i}$ and note
that
\begin{equation}
\E\left(\s{\X}-\X\right)^{2}=\sum_{\i=1}^{n}\frac{1}{n}\E\left(D_{\i}+R_{\i}\right)^{2}\le\sum_{\i=1}^{n}\frac{1}{n}\E\left(D_{\i}+1\right)^{2}=\E\left(D+1\right)^{2}.\label{eq:isolated-vertices-E(X^s-X)^2-bound-progress}
\end{equation}
The probability that a particular vertex is a degree-1 neighbour of
the vertex $\i$ is $p\left(1-p\right)^{n-2}$ because one particular
edge must be present and $n-2$ particular edges must be absent. So,
\begin{equation}
\E D=\left(n-1\right)p\left(1-p\right)^{n-2}=O\left(1\right).\label{eq:isolated-vertices-ED}
\end{equation}
The probability that two particular vertices are both degree-1 neighbours
of the vertex $\i$ is $p^{2}\left(1-p\right)^{2n-5}$, so 
\begin{alignat*}{1}
\E{D \choose 2} & =\E D^{2}/2-\E D/2={n-1 \choose 2}p^{2}\left(1-p\right)^{2n-5}=O\left(1\right)
\end{alignat*}
It follows from \ref{eq:isolated-vertices-E(X^s-X)^2-bound-progress}
that
\begin{equation}
\E\left(\s{\X}-\X\right)^{2}\le2\E{D \choose 2}+3\E D+1=O\left(1\right).\label{eq:isolated-vertices-E(X^s-X)^2-bound}
\end{equation}
Next, by the $L^{2}$ version of contractivity and the tower law (see
\ref{prop:tower-contractivity}), we have
\begin{equation}
...
Now, let $Y=\sum_{\i=1}^{n}D_{\i}$ be the number of vertices with
degree 1 in $G$, so that $\E^{G}\left[\s{\X}-\X\right]$ has the
distribution of $\frac{1}{n}\left(Y+n-\X\right)$. Let $\hat{Y}=Y-\E Y$
and $\hat{\X}=\X-\E\X$, so that
\[
\Var\left(\E^{G}\left[\s{\X}-\X\right]\right)=\frac{1}{n^{2}}\E(\hat{Y}-\hat{\X})^{2}=\frac{1}{n^{2}}\E\left[2\hat{Y}^{2}+2\hat{\X}^{2}-(\hat{Y}+\hat{\X})^{2}\right]\le\frac{2}{n^{2}}\left(\Var Y+\sigma^{2}\right).
\]
From \ref{eq:isolated-vertices-ED}, we can deduce 
\[
\E Y=n\left(n-1\right)p\left(1-p\right)^{n-2}.
\]
...
and $\ii$ has a single non-$\i$ neighbour (possibly both the same).
This event has probability $\left(n-2\right)^{2}p^{2}\left(1-p\right)^{2n-5}$.
So, 
\begin{alignat*}{1}
\E{Y \choose 2} & ={n \choose 2}\left(p\left(1-p\right)^{2n-4}+\left(n-2\right)^{2}p^{2}\left(1-p\right)^{2n-5}\right).
\end{alignat*}
It follows that $\Var Y=2\E{Y \choose 2}-\left(\E Y\right)^{2}+\E Y$
is equal to
\[
n\left(n-1\right)p\left(1-p\right)^{n-2}\left(\left(1-p\right)^{n-2}+p\left(1-p\right)^{n-3}\left(\left(n-2\right)^{2}-n\left(n-1\right)\right)+1\right)=O\left(n\right).
\]
Recalling that $\sigma^{2}=O\left(n^{2}\right)$, we have
\begin{alignat}{1}
\Var\left(\E^{G}\left[\s{\X}-\X\right]\right) & =O\left(\frac{1}{n}\right).\label{eq:isolated-vertices-var-full-bound}
\end{alignat}
Combining Equations \ref{eq:size-bias-normal-bound}, \ref{eq:isolated-vertices-E(X^s-X)^2-bound},
\ref{eq:isolated-vertices-var-contractive-bound}, \ref{eq:isolated-vertices-var-full-bound}
and \ref{eq:stein-distance-transform}, we conclude 
\[
...
Let $\lambda=\E\X$, and suppose we believe that $\L{\X}\approx\Po\left(\lambda\right)$.
Let $\T_{\Po\left(\lambda\right)}$ be the Poisson characterizing
operator defined in \ref{thm:poisson-characterizing}, and suppose
$f$ that satisfies the bounds in \ref{thm:poisson-bound}. We have
\[
...
to \ref{eq:poisson-limit-E-difference}, it follows that
\begin{equation}
\left|\E_{\X}\T_{\Po\left(\lambda\right)}f\right|\le\lambda\left\Vert \Delta f\right\Vert _{\infty}\E\left|\X+1-\s{\X}\right|\le(1-e^{-\lambda})\,\E\left|\X+1-\s{\X}\right|.\label{eq:size-bias-poisson-bound}
\end{equation}
...
and \ref{eq:stein-distance-transform} we have 
\begin{alignat*}{1}
d_{\TV}\left(\X,\Po\left(\lambda\right)\right) & \le(1-e^{-\lambda})\left(\left(1-p\right)^{n-1}+\left(n-1\right)p\left(1-p\right)^{n-2}\right)\\
 & \le\lambda(1-e^{-\lambda})\left(\frac{1}{n}+\frac{p}{1-p}\right).
\end{alignat*}
As we guessed in \ref{example:isolated-vertices}, this means $\X$
is approximately Poisson when $n$ is large and $p$ is not too close
to zero (or one), so that $\lambda$ is small and $\frac{p}{1-p}$
is not too big.
\end{example}
...
pairs construction can be used for this purpose, but in this chapter
...
this chapter, we will refer to an example problem to illustrate the
...
a time-homogeneous Markov chain $M$ with transition probabilities
...
distribution $\L{\X}$. Of course, given a reversible Markov chain
...
simplicial complex is trivial, and this motivates the construction
of a characterizing operator in a natural way. (The following theorem
is self-contained and requires no knowledge of simplicial homology).
\begin{thm}
...
by the $L^{1}$ version of contractivity, so $\TX\fX\in\XX$.
...
\ref{rem:indicator-expectation}, we have $\E{\X_{1} \choose 2}=\E\left[\X_{1}^{2}\right]/2-\E\X_{1}/2={n \choose 2}\left(n\left(n-1\right)\right)^{-1}=1/2$
...
\ref{fig:cycle-switching} illustrates the idea.
\end{defn}
\begin{figure}[h]
\begin{center}
\hspace{0.5cm}
...
\def \oN {10}
{
 \foreach \s in {1,...,\iN}
 {
  ...
  \node at ({90+360/\iN * (\s - 0.75)}:2.22cm){$u_\s$};
  \node at ({90+360/\iN * (\s - 1.25)}:2.22cm){$w_\s$};
 }
 \foreach \s in {1,...,\oN}
 {
  \node[vertex] (o\s) at ({360/\oN * (\s - 1)}:1.8cm){};% {$\s$};
 }
}
...
\protect\caption{\label{fig:cycle-switching}A switching with $\ell=5$ (unaffected
...
so that each vertex has the same weight $\Delta$. By \ref{prop:graph-to-markov},
...
\ref{rem:underlying-pair}, this provides us an exchangeable pair
$\bX=\left(\X_{1},\X_{2}\right)$ with margins $\L{\X}$, which we
use to apply Stein's method.
...
\E\left|\X_{1}-c\Pr\left(\X_{2}=\X_{1}+1|\X_{1}\right)\right| & \le\E\left|\sum_{C\in K_{n}}\one_{C\subseteq G_{1}}-\frac{c}{\Delta}\sum_{C\in K_{n}}F_{C}\left(G_{1}\right)\right|\nonumber \\
 & \le\frac{\falling n{\ell}}{2\ell}\E\left|\one_{C\subseteq G_{1}}-\frac{c}{\Delta}F_{C}\left(G_{1}\right)\right|.\label{eq:short-cycles-increment-bound}
\end{alignat}
Similarly, let $B_{C}\left(G\right)$ be the number of ways to switch
into $G$ by destroying $C$. We have 
\begin{equation}
\E\left|\frac{\left(d-1\right)^{\ell}}{2\ell}-c\Pr\left(\X_{2}=\X_{1}+1|\X_{1}\right)\right|\le\frac{\falling n{\ell}}{2\ell}\,\E\left|\frac{\left(d-1\right)^{\ell}}{\falling n{\ell}}-\frac{c}{\Delta}B_{C}\left(G_{1}\right)\right|.\label{eq:short-cycles-decrement-bound}
\end{equation}
...
\]
are small. The arguments needed to actually obtain bounds on these
expectations can be mostly adapted from corresponding arguments in
the bare-hands application of switchings in \cite{MWW04}, which we
will now sketch.
...
are distinct and no extraneous cycles are created or destroyed). A
choice will certainly be legal if we require that each of the edges
...
\]
Combining these estimates with \ref{eq:stein-distance-transform},
\ref{eq:short-cycles-increment-bound}, \ref{eq:short-cycles-decrement-bound}
and \ref{eq:EXT0-immigration-death-bound}, \ref{thm:short-cycles-variable}
...
used to make approximations to application-specific distributions
(see, for example, \cite{BCX07}). This is done using general characterizing
...
We have only discussed Stein's method as a tool for bounding the distance
between distributions, but the method can be applied to prove other
types of results, including, for example, concentration inequalities
(see, for example, \cite[Section~7]{Ros11}). The method has even
been used for totally non-probabilistic results, such as the asymptotic
enumeration of Latin rectangles (\cite[Section~XI]{Ste86}).
