...
\caption{\label{fig:isolated-vertices}Suppose the $G$ is the graph on the
left. The only isolated vertex is 5, so $\X=1$. Now, suppose $I=0$,
so all the edges adjacent to 0 are deleted to obtain the graph on
the right with $\s{\X}=3$ isolated vertices. The new isolated vertex
0 arose because $I$ was not isolated in $G$, and the new isolated
vertex 4 arose because 4 was a degree-1 neighbour of 0 in $G$.}
\end{figure}
