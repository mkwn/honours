...
\item put a remark in the Poisson section saying we'll do Poisson approximation
later
\item size-bias coupling
\item finish proof sketch for random regular graphs section - where does
the bound come from?
...
is the measure-theoretic integral $\E\X=\int x\d\L{\X}\left(x\right)$.\end{defn}
\begin{rem}
...
$\X_{1}$ and $\X_{2}$ are random variables defined on the same underlying
probability space $\left(\Om,\cA\right)$. Then\begin{enumerate}

\item (Tower Law) $\E\left[\E^{\X_{2}}\X_{1}\right]=\E\left[\X_{1}\right]$,

\item (Contractivity) $\E\left|\E^{\X_{2}}\X_{1}\right|\le\E\left|\X_{1}\right|$.

\end{enumerate}\end{prop}
\begin{proof}
Recall the definition of conditional expectation. We have
\[
\E\left[\E^{\X_{2}}\X_{1}\right]=\E\left[\E\left[\X_{1}|\X_{2}\right]\one_{\Om}\right]=\E\left[\X_{1}\one_{\Om}\right]=\E\left[\X_{1}\right].
\]
Next, let $\A=\left\{ \E^{\X_{2}}\X_{1}\ge0\right\} $, so
\[
\E\left|\E^{\X_{2}}\X_{1}\right|=\E\left[\one_{\A}\E^{\X_{2}}\X_{1}\right]-\E\left[\one_{\Om\backslash\A}\E^{\X_{2}}\X_{1}\right]=\E\left[\one_{\A}\X_{1}\right]-\E\left[\one_{\Om\backslash\A}\X_{1}\right]\le\E\left|\X_{1}\right|.\qedhere
\]
...
\global\long\def\Xs#1{#1^{\left(s\right)}}
...
integrable random variable $\X$ is given by $\Ls{\X}\left(A\right)=\E\left[\X\one_{\X\in\A}\right]/\E\X$.\end{defn}
\begin{rem}
...
\[
\Ls{\X}\left(k\right)=\frac{k}{\E\X}\Ls{\X}\left(k\right).
\]
This often has a combinatorial interpretation. Intuitively speaking,
if $\L{\X}$ is the number of balls in a random bin, then $\Ls{\X}$
is the distribution of the number of balls in the bin of a random
ball.
\end{rem}
We can immediately see from \ref{rem:size-bias-discrete} that in
the discrete case $\E\fo\left(\Xs{\X}\right)=\E\left[\X\fo\left(\X\right)\right]/\E\X$
...
for Stein's method, we need to couple $\X$ with $\Xs{\X}$ in an
appropriate way. The objective of this coupling varies slightly between
the Normal and Poisson cases, but there is a general construction
that can be frequently used for both.
\begin{thm}
coming soon
\end{thm}
