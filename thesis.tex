...
In this thesis, we will first set out the theoretical groundwork for
Stein's method, including an overview of basic probability theory
and a discussion of weak convergence (\ref{sec:probability-theory}).
We then present a very general framework for the Stein's method (\ref{sec:stein-general})
before specializing to the Normal and Poisson cases (\ref{sec:normal,sec:poisson}).
We finally outline a few particular ways of applying the method (\ref{sec:size-bias-coupling,sec:exchangeable-pairs}).
...
We will occasionally use big-oh notation non-asymptotically, to avoid
writing constant factors.
...
\]
for any Borel set $B$. We can use the Gaussian integral to show that
this is indeed a probability measure, with $\N_{\mu,\sigma}\left(\RR\right)=1$.
...
Also, note that $\frac{\X-\mu}{\sigma}\in\N\left(0,1\right)$. So,
we will sometimes only consider the \emph{standard normal} distribution
...
\begin{rem}
For a random variable $\X$ that takes integer values, our definition
of expected value is equivalent to the well-known formula $\E\X=\sum_{x\in\ZZ}x\,\Pr\left(\X=x\right)$.
\end{rem}
...
a sub-$\sigma$-algebra $\sigma\left(\X_{2}\right)$ of $\cA$. Then,
$\m:\A'\mapsto\E\left[\X_{1}\one_{\A'}\right]$ is a signed measure
on $\sigma\left(\X_{2}\right)$ that is absolutely continuous with
...
this means $\X_{3}$ is dependent only on the information from $\X_{2}$).
Then\begin{enumerate}

\item (Tower Law) $\E^{\X_{3}}\left[\E^{\X_{2}}\X_{1}\right]=\E^{\X_{3}}\left[\X_{1}\right]$.
In particular, taking $\X_{3}\equiv1$, we have $\E\left[\E^{\X_{2}}\X_{1}\right]=\E\left[\X_{1}\right]$.
...
$\Lb{\X_{n}}\to\Lb{\X}$.
\end{defn}
This is only one of a number of equivalent natural definitions for
...
method allows for a very simple proof.
...
\E\left[\X_{\Po\left(\lambda\right)}^{k}\left|\U_{\Po\left(\lambda\right)}\h\left(\X_{\Po\left(\lambda\right)}\right)\right|\right] & =O\left(\sum_{\i=1}^{\infty}\i^{k-1}\left|\sum_{\ii=0}^{\i-1}\frac{\lambda^{\ii}}{\ii!}\left(\h\left(\ii\right)-\E_{\Po\left(\lambda\right)}\h\right)\right|\right)\\
 ...
 & =O\left(\sum_{\ii=1}^{\infty}\ii^{k}\frac{\lambda^{\ii}}{\ii!}\left|\h\left(\ii\right)-\E_{\Po\left(\lambda\right)}\h\right|\right)\\
 & <\infty,
\end{alignat*}
...
\end{rem}
We will see some examples of Poisson approximation using Stein's method
...
of the form $\E\left[\X f\left(\X\right)\right]$. In \ref{sec:exchangeable-pairs},
...
by that value. To be precise, in the discrete the definition of $\Ls{\X}$
can be stated as 
\begin{equation}
...
the discrete case 
\begin{equation}
\E\fo\left(\s{\X}\right)=\E\left[\X\fo\left(\X\right)\right]/\E\X\label{eq:size-bias-transformation}
\end{equation}
...
way. There is a general construction that can be frequently used for
both normal and Poisson approximation in combinatorial applications.
\begin{prop}
Let $\left(Q_{\i}\right)_{\i=1}^{n}$ be a sequence of zero-one random
variables, and let $\X=\sum_{\i=1}^{n}Q_{\i}$. Suppose \textup{$\left(\X^{\left(\i\right)}\right)_{\i=1}^{n},\left(Q_{\i}\right)_{\i=1}^{n}$
...
We have 
\begin{alignat*}{1}
\Pr\left(\X^{\left(I\right)}=k\right) & =\sum_{\i=1}^{n}\frac{\Pr\left(Q_{\i}=1\right)}{\E\X}\Pr\left(\X=k|Q_{\i}=1\right)\\
...
Fix $n$ and $p$, and let $G\in\rG np$ (see \ref{sec:random-structures}).
Let 
\[
...
to have an approximate Poisson distribution. If $p$ vanishes at just
the right rate (compared to $n$), then we might expect the central
limit theorem to approximately hold, so that $\X$ has an approximate
normal distribution. We will therefore return to this example to illustrate
...
do this now. Using \ref{rem:indicator-expectation}, we can immediately
deduce that $\E\X=n\left(1-p\right)^{n-1}$. For a pair of distinct
vertices, the probability that both are isolated is $\left(1-p\right)^{2n-3}$,
so
\[
\E\left[\X\left(\X-1\right)/2\right]={n \choose 2}\left(1-p\right)^{2n-3}
\]
and
\begin{alignat*}{1}
\Var\X & =n\left(n-1\right)\left(1-p\right)^{2n-3}+n\left(1-p\right)^{n-1}-n^{2}\left(1-p\right)^{2n-2}\\
 & =n\left(1-p\right)^{n-1}\left(1+\left(np-1\right)\left(1-p\right)^{n-2}\right).
\end{alignat*}
...
\]
for some (random) $E$. So, for $\To=\T_{\N}$ as defined in \ref{thm:normal-characterizing},
we have 
\begin{equation}
\left|\E_{W}\To f\right|\le\frac{3\mu}{\sigma^{2}}\sqrt{\frac{\pi}{2}}\E\left|\frac{\sigma^{2}}{\mu}-\E^{\X}\left[\s{\X}-\X\right]\right|+\frac{3\mu}{\sigma^{3}}\E\left(\s{\X}-\X\right)^{2}.\label{eq:size-bias-normal-bound}
\end{equation}
...
$L^{2}$ norm then gives
\begin{equation}
\E\left|\frac{\sigma^{2}}{\mu}-\E^{\X}\left[\s{\X}-\X\right]\right|\le\sqrt{\Var\left(\E^{\X}\left[\s{\X}-\X\right]\right)}.\label{eq:size-bias-normal-variance-bound}
\end{equation}


It follows that $d_{\W}\left(\X,\N\left(0,1\right)\right)$ will be
small when $\s{\X}$ is coupled in such a way that it is a slight
...
\begin{example}
As in \ref{example:isolated-vertices}, let $\X$ be the number of
isolated vertices in a random $G\in\rG np$, let $I$ be a random
vertex, and let $\s{\X}$ be the number of isolated vertices with
the edges incident to a random vertex deleted. For each vertex $\i$
in $G$, let $D_{\i}$ be the number of vertices adjacent to $\i$
which have degree $1$, and as before let $Q_{\i}$ be the indicator
random variable for the event that $\i$ is isolated. Then, we have
\[
\s{\X}-\X=D_{I}+Q_{I}.
\]
First, note that
\begin{equation}
\E\left(\s{\X}-\X\right)^{2}=\sum_{\i=1}^{n}\frac{1}{n}\E\left(D_{\i}+Q_{\i}\right)^{2}\le\sum_{\i=1}^{n}\frac{1}{n}\E\left(D_{\i}+1\right)^{2}=\E D_{\i}^{2}+2\E D_{\i}+1.\label{eq:isolated-vertices-E(X^s-X)^2-bound}
\end{equation}
...
That is,
\[
\E^{G}\left[\s{\X}-\X\right]=\sum_{\i=1}^{n}\frac{1}{n}\left(D_{\i}+Q_{\i}\right).
\]
Now, let $D=\sum_{\i=1}^{n}D_{\i}$ be the number of vertices with
degree 1 in $G$, and let $\hat{D}=D-\E D$. Recall that $\X=\sum_{\i=1}^{n}Q_{\i}$,
and let $\hat{\X}=\X-\E\X$. We have
\begin{equation}
\Var\left(\E^{G}\left[\s{\X}-\X\right]\right)=\frac{1}{n^{2}}\E\left(\hat{D}+\hat{\X}\right)^{2}=\frac{1}{n^{2}}\E\left[2\hat{D}^{2}-2\hat{\X}^{2}-\left(\hat{D}-\hat{\X}\right)^{2}\right]\le\frac{2}{n^{2}}\left(\Var D+\Var\X\right).\label{eq:isolated-vertices-var-full-bound}
\end{equation}
Combining Equations \ref{eq:size-bias-normal-bound}, \ref{eq:size-bias-normal-variance-bound},
\ref{eq:isolated-vertices-E(X^s-X)^2-bound}, \ref{eq:isolated-vertices-var-contractive-bound}
and \ref{eq:isolated-vertices-var-full-bound}, we just need to estimate
$\Var D$, $\E D_{i}$ and $\E D_{i}^{2}$ in order to obtain a bound
for 
\[
d_{\W}\left(\frac{\X-\mu}{\sigma},\N\left(0,1\right)\right).
\]
...
\]
so $\E_{\X}\To\fo=\E_{\X}\left(\To\fo-\TX\conn\fX\right)$ equals
\[
...
\begin{equation}
\left|\E_{\X}\To\fo\right|\le\min\left(2,3\lambda^{-1/2}\right)\left(\E\left|\lambda-c\Pr\left(\X_{2}=\X_{1}+1|\X_{1}\right)\right|+\E\left|\X_{1}-c\Pr\left(\X_{1}=\X_{2}+1|\X_{1}\right)\right|\right)\label{eq:EXT0-immigration-death-bound}
\end{equation}
for all $\fo$ satisfying the bound in \ref{example:poisson-bound}.
