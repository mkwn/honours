...
space.
\begin{defn}
The notation $\X\in\L{}$ means $\L{\X}=\L{}$.
\end{defn}
We can use slightly abusive (but standard) notation like $\Pr\left(\X>1\right)$
...
$\i$th object satisfies our property. We have $\E\X=\sum_{\i}\E\one_{\A_{\i}}=\sum_{\i}\Pr\A_{\i}$.
So, in order to compute the expectation of $\X$ we just need to compute
...
for all $\fo\in\cY$: 
\[
\E_{\X}\To\fo\le\min\left(1,\lambda^{-1/2}\right)\left(\E\left|1-c\Pr\left(\X_{2}=\X_{1}+1|\X_{1}\right)\right|+\E\left|X_{1}-c\Pr\left(\X_{1}=\X_{2}+1|\X_{1}\right)\right|\right).
\]
