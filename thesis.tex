...
Let $\cX_{\N}$ be the set of functions $\h:\RR\to\RR$ that satisfy
$\E_{\N}\left|\h\right|<\infty$ and let $\cF_{\N}$ be the set of
functions $\fo:\RR\to\RR$ such that $\E_{\N}\left|\T_{\N}\fo\right|<\infty$.
...
and let $\cF_{\Po\left(\lambda\right)}$ be the set of functions $\fo:\NN\to\ZZ$
such that $\E_{\Po\left(\lambda\right)}\left|\T_{\Po\left(\lambda\right)}\fo\right|<\infty$.
...
\begin{example}
If $\cH=\cH_{\TV}$ and $\Lo=\Po\left(\lambda\right)$, then we can
choose $\cY=\left\{ \fo\in\Fo:\left\Vert \fo\right\Vert _{\infty}\le2\min\left\{ 1,\lambda^{-1/2}\right\} \right\} $.
...
\begin{proof}
(Adapted from \cite[Lemma 1.1.1]{BHJ92})...\begin{comment} First,
note that $\left\Vert \U_{\Po\left(\lambda\right)}\one_{\range{k-1}}\right\Vert _{\infty}$
...
so
\[
\left|\U_{\Po\left(\lambda\right)}\one_{\A}\left(k\right)\right|\le\frac{\left(k-1\right)!}{\lambda^{k}}\Po_{\lambda}\left(\range{k-1}\right)\Po_{\lambda}\left(\RR\range{k-1}\right).
\]
We have equ\end{comment}
\end{proof}
\begin{todo}
...
is then finite. We write its elements as $\x 1<\dots<\x k$ for $k=\left|\OmX\right|$.
...
\end{todo}
\begin{defn}
A 2-dimensional random vector $\eX=\left(\X_{1},\X_{2}\right)$ is
an \emph{exchangeable pair} if $\Lb{\X_{1},\X_{2}}=\Lb{\X_{2},\X_{1}}$.
\end{defn}
...
We will be interested in the case when $\L{\X_{1}}=\L{\X_{2}}=\L{\X}$.
...
$p_{\i\ii}=\Pr\left(\X_{2}=\x{\ii}|\X_{1}=\x{\i}\right)$. With $\pi_{\i}=\Pr\left(\X_{1}=\x{\i}\right)$,
we then have
\[
\pi_{\i}p_{\i\ii}=\Pr\left(\eX=\left(x_{\i},x_{\ii}\right)\right)=\pi_{\ii}p_{\ii\i},
\]
so $M$ is reversible with steady-state distribution $\L{\X}$.

Conversely, suppose a time-homogeneous Markov chain with transition
probabilities $p_{\i\ii}$ satisfies $\pi_{\i}p_{\i\ii}=\pi_{\ii}p_{\ii\i}$
for $\pi_{\i}=\Pr\left(\X_{1}=\x{\i}\right)$. We can then define
an exchangeable pair $\eX$ by 
\[
\Pr\left(\eX=\left(\x{\i},\x{\ii}\right)\right)=\pi_{\i}p_{\i\ii}=\pi_{\ii}p_{\ii\i}=\Pr\left(\eX=\left(\x{\ii},\x{\i}\right)\right).
\]

\end{proof}
For any exchangeable pair $\eX$, we can also define its graph $\GX$
in a natural way: the vertex set $\range k$ corresponds to $\OmX=\left\{ \x 1,\dots,\x k\right\} $,
and there is an edge for each possible transition in the Markov chain
of $\eX$. We will be particularly interested in the interpretation
of $\GX$ as an (abstract) simplicial complex. That is, the set of
one-dimensional faces $F_{1}$ is the edge set of $\GX$ and the set
of zero-dimensional faces $F_{0}$ is the vertex set $\range k$ of
$\GX$.
\begin{defn}
We say an exchangeable pair $\eX$ is \emph{connected} if its Markov
chain is irreducible, or equivalently $\GX$ is connected.
\end{defn}
...
reversible Markov chain with steady-state distribution given by $\pi=\Pr\left(\om_{\i}\right)$,
...
Now, if $\eX$ is a connected exchangeable pair with margins $\L{\X}$,
consider the augmented chain complex of $\GX$ over $\RR$:
\[
0\to\RR^{F_{1}}\xrightarrow{\partial_{1}}\RR^{F_{0}}\xrightarrow{\partial_{0}}\RR\to0.
\]
Since $\GX$ is connected, its zeroth homology group has dimension
zero. That is, $\im\partial_{1}=\ker\partial_{0}$. This motivates
the construction of a characterizing operator for $\X$.

Let $\FX$ be the set of functions $\OmX^{2}\to\RR$ that are antisymmetric
in the sense that $\fX\left(x_{1},x_{2}\right)=-\fX\left(x_{2},x_{1}\right)$,
and let $\XX$ be the set of functions $\OmX\to\RR$.
\begin{thm}
Let $\FX$ and $\XX$ be defined as above. Suppose\textup{ $\eX$
...
We next prove $\ker\E_{\X}\subseteq\im\TX$ using the fact that $\im\partial_{1}=\ker\partial_{0}$.
A chain in $\RR^{F_{\i}}$ will be represented as a function $F_{\i}\to\RR$.

Let $\hX\in\ker\E_{\X}$. That is, $\sum_{\i=1}^{k}\hX\left(\x{\i}\right)\pi_{\i}=0$,
so if we define $\hX^{*}:\i\mapsto\hX\left(\x{\i}\right)\pi_{\i}$,
then $\hX^{*}\in\ker\partial_{0}=\im\partial_{1}$. This means there
is $\fX^{*}\in\RR^{F_{1}}$ with $\partial_{1}\fX^{*}=\hX^{*}$, or
equivalently
\[
\hX^{*}\left(\i\right)=\sum_{\ii\in\range k\backslash\left\{ \i\right\} }\fX^{*}\left(\left\{ \i,\ii\right\} \right)\sgn\left(\i-\ii\right)
\]
for each $\i$. Now, define $\fX:\OmX^{2}\to\RR$ by 
\[
\fX\left(\x{\i},\x{\ii}\right)=\begin{cases}
{\displaystyle \frac{\fX^{*}\left(\left\{ \i,\ii\right\} \right)\sgn\left(\i-\ii\right)}{\pi_{\i}p_{\i\ii}}} & \mbox{if }\left\{ \i,\ii\right\} \in F_{1}\\
0 & \mbox{otherwise}.
\end{cases}
\]
By the reversibility of the Markov chain of $\eX$, the function $\fX$
is antisymmetric so $\fX\in\FX$. By construction, we have
\[
\hX\left(\i\right)=\sum_{\ii\in\range k\backslash\left\{ \i\right\} }\fX\left(\x{\i},\x{\ii}\right)p_{\i\ii}=\TX\fX\left(\i\right)
\]
for each $\i$, so $\hX\in\im\TX$.
\end{proof}
