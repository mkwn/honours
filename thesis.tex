...
up with them, although many are quite standard and probably exist
elsewhere. For the proofs that I have adapted from other sources,
...
Also, $\range k$ denotes the set $\left\{ 1,\dots,k\right\} $. The
function $f$ restricted to the set $A$ is denoted $f\rest_{A}$.
...
I will use results from analysis, particularly measure theory, without
proof. I will usually refer to a numbered theorem in Rudin's Real
and Complex Analysis \cite{Rud66} or Functional Analysis \cite{Rud73}
when doing so.
...
is $\E\X=\int x\d\L{\X}\left(x\right)$.
\end{defn}
...
Convergence in distribution of random vectors is defined component-wise.
\end{defn}
...
I haven't defined limit theorems
...
\begin{prop}
Each $d_{\cH}$ is a metric.\end{prop}
\begin{proof}
...
the definition of $d_{\cH}$ is finite as well. It is immediate that
$d_{\cH}$ is non-negative, symmetric and satisfies the triangle inequality.
...
first moment then $d_{\W}\left(\Pr_{1},\Pr_{2}\right)\le2\E_{\Pr}\left|\id_{\RR}\right|<\infty$.

Now, we just need to check that each of $\cH_{\K},\cH_{\BL},\cH_{\TV},\cH_{\W}$
are determining classes. Let $\Pr_{1},\Pr_{2}\in\P\left(\RR\right)$
...
and each $\h_{x,1/n}\le1$. By the dominated convergence theorem,
\[
...
\]
so $\cH_{\BL}$ and $\cH_{\W}$ are determining classes.\end{proof}
\begin{prop}
...
we have
\[
\Pr\left(\left(-k,k\right)\right)\le\liminf_{n\to\infty}\Pr_{k_{n}}\left(\left(-k_{n},k_{n}\right)\right)\le\liminf_{n\to\infty}\Pr_{n}\left(\left(-n,n\right)\right)\le1-\varepsilon.
\]
This is a contradiction because $\Pr\left(\left(-k,k\right)\right)=\E_{\Pr}\one_{\left(-k,k\right)}\to\E_{\Pr}1=1$
by the dominated convergence theorem (see \cite[Theorem 1.34]{Rud66}).
\end{proof}
