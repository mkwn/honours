...
This paper gives a higher-level overview of Stein's method with exchangeable
pairs. If we think $X$ is ``near'' $Z$, then the operators $\mathbb{E}:f\mapsto\mathbb{E}\left[f\left(X\right)\right]$
and $\mathbb{E}_{0}:f\mapsto\mathbb{E}\left[f\left(Z\right)\right]$
should also be close. An exchangeable pair $\left(X,X'\right)$ is
in correspondence with a symmetric measure $\mathbb{Q}$ with $\mathbb{Q}\left(A,A'\right)=\Pr\left(X\in A,X'\in A'\right)$
and margins $\mathbb{Q}\left(A,\Omega\right)=\Pr\left(X\in A\right)$.
Let $T_{0}$ ($\mathcal{A}$ from before) be the characterizing operator
of $Z$. (unfinished)
